...
class NAVSTAT():
	...
	def start(self):
		...
		with open("gps_data.csv", "w", 128, newline='') as log_file:
        	log_file_writer = csv.writer(log_file, delimiter=',')
        	colnames = ["lat", "lon", "speed", "track", "utc", "status"]
			...
			while self.exit == False:
				...
				if self.navstat_mode == 0:
					if self.nmea.exit == False:
						self.nmea.data_gps['lat'] == 0 and self.nmea.data_gps['lon']
						...
						log_file_writer.writerow([self.nmea.data_gps['lat'], self.nmea.data_gps['lon'], self.nmea.data_gps['speed'], self.nmea.data_gps['track'], self.nmea.data_gps['utc'], self.nmea.data_gps['status']])
						self.gps.interface()
						self.gps.latlong()
